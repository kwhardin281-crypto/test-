...
# --------------- Utilities ---------------
def _to_month(ts): return pd.to_datetime(ts).to_period("M").to_timestamp()
...
# --------------- Feature Builder ---------------
DEFAULT_NUMERIC = ["beds","baths","sqft","year_built","amenities_score","condition_score","renovation_year","crime_index","school_score"]
...
def build_snapshot(prop, zori):
    ...
    snap = pd.merge(
        ...
        left_on="as_of_month", right_on="date", how="left"
    ).drop(columns=["date"])
    # Autodetect numeric features if present
    num_cols = [c for c in DEFAULT_NUMERIC if c in snap.columns]
    X_cols = TREND_COLS + num_cols
    X = design_matrix(snap.assign(intercept=1), ["intercept"] + X_cols)  # intercept first arg used only for shape
    # But design_matrix above expects real X_cols; rebuild correctly:
    X = np.column_stack([np.ones(len(snap))] + [snap[c].astype(float).fillna(0).values for c in X_cols])
...
def project_units(snap, zori, beta, X_cols, cfg):
    ...
    for _, u in snap.iterrows():
        feat = {c: float(u[c]) if c in u and pd.notna(u[c]) else 0.0 for c in X_cols}
        Xh = np.column_stack([
            np.ones(len(horizon)),
            zh["t"].values, zh["zori_norm"].values
        ] + [
            np.full(len(horizon), feat.get(c, 0.0)) for c in X_cols if c not in TREND_COLS
        ])
        ...
        yearly = df.groupby(["property_id","unit_id","year"], as_index=False)["rent_baseline"].mean()
        rows.append(yearly)
    ...
    for r in infls:
        ...
        def grow(series):
            ...
            for i in range(1, len(vals)):
                g = r
                if cap is not None: g = min(g, float(cap))
                vals[i] = vals[i-1]*(1.0+g)
            return pd.Series(vals, index=series.index)
        out[col] = out.groupby(["property_id","unit_id"])["rent_baseline"].transform(grow)
    return out

# --------------- Backtest (simple) ---------------
def backtest(prop, zori, holdout_months=12):
    # Build a pseudo time-series by cloning each unit's current_rent backwards with zori deltas (toy example)
    # Then train on pre-holdout and predict the holdout, compute metrics.
    z = zori.sort_values("date").copy()
    # Define split
    ...
    # Create synthetic historical unit rents aligned to zori (baseline for demo)
    # Assume unit rent scales with ZORI ratio relative to as_of_month
    prop = prop.copy()
    prop["as_of_month"] = prop["as_of_date"].dt.to_period("M").dt.to_timestamp()
    ...
    # Join features from zori for each timestamp
    ...
    def Xy(df):
        X = np.column_stack([np.ones(len(df))] + [df[c].astype(float).fillna(0).values for c in X_cols])
    ...
    # Baselines
    # 1) Flat baseline (hold last train rent per unit)
    last_train = train.sort_values("date").groupby(["property_id","unit_id"])["rent"].tail(1)
    last_map = train.sort_values("date").groupby(["property_id","unit_id"])["rent"].last().to_dict()
    flat_pred = []
    for idx, row in test.iterrows():
        key = (row["property_id"], row["unit_id"])
        flat_pred.append(last_map.get(key, ytr.mean()))
    flat_pred = np.array(flat_pred)

    # 2) ZORI-only baseline: regress rent on zori_norm + intercept using train
    ...
    # Also return a small sample frame for plotting or inspection
...
def main():
    ...
    ap.add_argument("--holdout_months", type=int, default=12)
    
    ap.add_argument("--open_summary", action="store_true", help="Open the newest summary CSV after writing outputs")
    ap.add_argument("--export_charts", action="store_true", help="Export PNG charts of YoY rent by scenario")
    ap.add_argument("--no_backtest", action="store_true", help="Skip the time-split backtest for faster runs")
    ...
    if bt is not None:
        ...
        print("Model MAE:", metrics["model_mae"], " vs Flat MAE:", metrics["flat_mae"], " vs ZORI-only MAE:", metrics["zori_only_mae"])
        print("Wrote:", outdir / f"metrics_{ts}.csv", outdir / f"backtest_preds_{ts}.csv")
    else:
        print("Not enough history to backtest.")

    print("Wrote:", outdir / f"forecast_plus_{ts}.csv", outdir / f"summary_plus_{ts}.csv")

    # Optional: export charts (one per scenario)
    ...
    # Optional: open newest summary CSV
